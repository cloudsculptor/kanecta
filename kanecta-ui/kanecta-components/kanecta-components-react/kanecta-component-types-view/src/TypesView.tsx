...
export interface TypesViewProps {
  ...
  onFetchStats: () => Promise<{ structured: Array<{ typeId: string; count: number }> }>;
  onFetchSchema: (typeId: string) => Promise<unknown>;
  onSaveSchema: (typeId: string, schema: unknown) => Promise<unknown>;
...
function MetaEditor({ typeId, schema, onSchemaChange, onSaveSchema }: MetaEditorProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onSaveSchema(typeId, updated);
    ...
    <div className="TypesView-editwrap">
      <div className="TypesView-metaform">
        <div className="TypesView-field">
          ...
          <input className="TypesView-input" value={fields.icon} onChange={set('icon')} placeholder="e.g. TaskAlt" spellCheck={false} />
        ...
        <div className="TypesView-field">
          ...
          <input className="TypesView-input" value={fields.description} onChange={set('description')} placeholder="One-sentence summary" />
        </div>
        <div className="TypesView-field">
          <label className="TypesView-label">Details</label>
          <textarea className="TypesView-textarea" value={fields.details} onChange={set('details')} rows={5} placeholder="Longer description of this type, when to use it, and how it relates to other types" />
        </div>
        <div className="TypesView-field">
          <label className="TypesView-label">Keywords</label>
          <input className="TypesView-input" value={fields.keywords} onChange={set('keywords')} placeholder="Space-separated keywords for search" spellCheck={false} />
        </div>
        <div className="TypesView-field">
          <label className="TypesView-label">Tags</label>
          <input className="TypesView-input" value={fields.tags} onChange={set('tags')} placeholder="Comma-separated tags" spellCheck={false} />
        </div>
        <div className="TypesView-field">
          <label className="TypesView-label">AI Instructions — Claude</label>
          <textarea className="TypesView-textarea" value={fields.claude} onChange={set('claude')} rows={6} placeholder="Guidance for Claude on when and how to use this type" />
        </div>
      </div>
      <div className="TypesView-toolbar">
        ...
        <div className="TypesView-toolbar-actions">
          <button className="TypesView-btn TypesView-btn--primary" onClick={() => void handleSave()}>Save</button>
        </div>
      </div>
    </div>
  );
...
function DetailPane({ type, schema, onSchemaChange, onSaveSchema, onFetchMetadata, typeSpec, initialTab = 'view' }: DetailPaneProps) {
  ...
  const handleSave = async () => {
    ...
    setEditText(result);
    try {
      await onSaveSchema(type.id, JSON.parse(result));
  ...
  return (
    <div className="TypesView-detail">
      ...
      <div className="TypesView-tabcontent">
        ...
        {tab === 'meta-edit' && (
          <MetaEditor key={type.id} typeId={type.id} schema={schema} onSchemaChange={onSchemaChange} onSaveSchema={onSaveSchema} />
        )}
        {tab === 'schema' && (
          <textarea className="TypesView-schema" value={extractSection(schema, 'jsonSchema')} readOnly spellCheck={false} />
        )}
        {tab === 'reference' && (
          <textarea className="TypesView-schema" value={typeSpec ?? ''} readOnly spellCheck={false} />
        )}
        {tab === 'edit' && (
          <div className="TypesView-editwrap">
            <textarea
...
export function TypesView({
  ...
  const { data: types = [], isLoading: typesLoading } = useQuery({
    queryKey: ['types', queryKey],
    queryFn: onFetchTypes,
  });

  const { data: stats } = useQuery({
    queryKey: ['items-stats', queryKey],
  ...
  const handleSelect = async (t: TypeItem) => {
    setSelectedType(t);
    setSelectedInitialTab('view');
    try {
      const s = await onFetchSchema(t.id);
  ...
  const handleCreate = async () => {
    ...
    setAddError(null);
    try {
      const newType = await onCreateType(newName.trim());
      await queryClient.invalidateQueries({ queryKey: ['types', queryKey] });
  ...
  return (
    <div className="TypesView">
      ...
      <div className="TypesView-columns">
        ...
        {selectedType ? (
          <DetailPane
            key={`${selectedType.id}-${selectedInitialTab}`}
            type={selectedType}
            schema={schema}
