...
export interface SyncTypesDialogProps {
  open: boolean;
  onClose: () => void;
...
export function SyncTypesDialog({
  open,
  onClose,
  onFetchSystemTypes,
  onFetchTypes,
  ...
  const { data: systemItems = [], isLoading: loadingCommon } = useQuery({
    queryKey: ['sync-system-items', queryKey],
    queryFn: onFetchSystemTypes,
    enabled: open,
  });
  ...
  const instanceIds = new Set(instanceTypes.map((t: TypeItem) => t.id));
  const commonFolderIds = new Set(systemItems.map((c: SystemTypeItem) => c.folderId));

  const notInInstance = systemItems.filter((c: SystemTypeItem) => !instanceIds.has(c.folderId));
  const notInCommon = instanceTypes.filter((t: TypeItem) => !commonFolderIds.has(t.id));
  const inSyncCount = systemItems.filter((c: SystemTypeItem) => instanceIds.has(c.folderId)).length;
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      await onImportTypes(ids);
      await qc.invalidateQueries({ queryKey: ['types', queryKey] });
