...
- [1.3.0](1.3.0/README.md) — filesystem datastore migration from spec v1.2.0
  to v1.3.0 (`migrate-1.2.0-to-1.3.0.ts` + an AI-assisted data-reshaping
  runbook for the parts that need human judgement).
- [1.4.0](1.4.0/README.md) — filesystem datastore migration from spec v1.3.0
  to v1.4.0. A two-step chain: `migrate-1.3.0-to-1.4.0.ts` merges split
  ...
  `migrate-datastore-to-per-branch.ts` restructures the datastore into the
  per-branch full-folder layout. `migrate-config-keys.ts` migrates the versioned
