...
| [core-file-specs/config.json](core-file-specs/config.json) | JSON Schema for `config.json` — the bootstrap config that tells the platform where datastores live (working sets) and how to connect to remotes. |
| [core-file-specs/branch.json](core-file-specs/branch.json) | JSON Schema for `branch.json` — per-branch metadata (fill: full/sparse, upstream, branchPoint) stored inside a datastore at `branches/<name>/`. |
| [core-file-specs/state.json](core-file-specs/state.json) | JSON Schema for `state.json` — machine-local active working set + branch (the HEAD), stored beside `config.json`. |
