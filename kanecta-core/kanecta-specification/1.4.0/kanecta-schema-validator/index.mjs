...
const isISO8601 = v => typeof v === 'string' && ISO8601_RE.test(v);
const isDate    = v => typeof v === 'string' && DATE_RE.test(v);
const e = (path, message, rule) => ({ path, message, rule });
 ...
 */
export function validateType(typeJson) {
  const errors = [];
 ...
 * @returns {{ valid: boolean, errors: Array<{path:string, message:string, rule:string}> }}
 */
export function validateItem(data, typeJson) {
  const errors = [];
