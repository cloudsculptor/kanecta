...
import { buildSchemaModel, emitSDL, resolverPlan } from '../../src/graphql/index.ts';
import {
  allTypes,
  chThreadType,
  chMessageType,
...
describe('buildSchemaModel', () => {
  ...
  it('preserves snake_case column field names verbatim', () => {
    const thread = model.types.find((t) => t.name === 'ChThread')!;
  ...
  it('maps date-time columns to the DateTime scalar', () => {
    const thread = model.types.find((t) => t.name === 'ChThread')!;
    expect(thread.fields.find((f) => f.name === 'created_at')!.graphqlType).toBe('DateTime');
    expect(model.customScalars).toContain('DateTime');
  });

  it('maps integer columns to Int', () => {
    const thread = model.types.find((t) => t.name === 'ChThread')!;
    expect(thread.fields.find((f) => f.name === 'sort_order')!.namedType).toBe('Int');
  });

  it('hides columns marked x-graphql.expose:false', () => {
    const thread = model.types.find((t) => t.name === 'ChThread')!;
    expect(thread.fields.find((f) => f.name === 'archived_at')).toBeUndefined();
    const message = model.types.find((t) => t.name === 'ChMessage')!;
    expect(message.fields.find((f) => f.name === 'deleted_at')).toBeUndefined();
  });

  it('maps a typeId FK property to a reference field targeting the object type', () => {
    const message = model.types.find((t) => t.name === 'ChMessage')!;
    const threadId = message.fields.find((f) => f.name === 'thread_id')!;
  ...
  it('builds per-viewer computed fields backed by a function item', () => {
    const thread = model.types.find((t) => t.name === 'ChThread')!;
    const hasUnread = thread.fields.find((f) => f.name === 'has_unread')!;
...
describe('buildSchemaModel options', () => {
  ...
  it('flags GraphQL type-name collisions as errors', () => {
    const dupe = { ...chFileClone(), };
    const model = buildSchemaModel([chThreadType, dupe]);
    expect(model.diagnostics.some((d) => d.level === 'error' && /collides/.test(d.message))).toBe(true);
  });
});

function chFileClone() {
...
describe('resolverPlan', () => {
  ...
  it('indexes every field backing by TypeName.fieldName', () => {
    ...
    expect(plan.get('ChMessage.thread_id')).toMatchObject({ kind: 'reference', column: 'thread_id' });
    expect(plan.get('ChThread.messages')).toMatchObject({ kind: 'containment', targetTypeName: 'ChMessage' });
    expect(plan.get('ChThread.has_unread')).toMatchObject({ kind: 'computed', scope: 'perViewer' });
  });
});
...
describe('emitSDL', () => {
  ...
  it('emits an object type with snake_case fields', () => {
    expect(sdl).toMatch(/type ChThread \{/);
    expect(sdl).toMatch(/created_by_user_id: DateTime|created_by_user_id: String/);
  ...
  it('emits a where input with boolean combinators and scalar filters', () => {
    ...
    expect(sdl).toMatch(/created_at: DateTimeFilter/);
  });
