...
`metadata.json` requires every record to carry `specVersion` (e.g. `'1.4.0'`) — the version of the Kanecta specification that record's shape conforms to, recorded at creation time so tooling can pick the right schema/migration rules when reading older records. Migration `017_item_spec_version.sql` adds it as a plain column on `items`:

```sql
ALTER TABLE items ADD COLUMN IF NOT EXISTS spec_version TEXT NOT NULL DEFAULT '1.4.0';
```

Existing rows are backfilled to `'1.4.0'` (the version they were created under); `create()`/`createType()` stamp new rows with the running `@kanecta/specification` package's `version`. This is distinct from the singleton `schema_version` table below, which tracks the *database schema's* version, not individual items' spec conformance.
